...
Every response follows the OpenAI Chat Completions format, regardless of provider. â
...
ð [Full output format reference â](./completion/output)
...
"Observability callbacks (Langfuse, MLflow, Heliconeâ¦)",
],
...
ð [See all observability integrations â](/docs/observability/opentelemetry_v2)
...
ð [Custom callback docs â](./observability/custom_callback)
...
ð [Full proxy quickstart â](./proxy/docker_quick_start)
...
ð [Interactive API explorer (Swagger) â](https://litellm-api.up.railway.app/)
